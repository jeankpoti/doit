...
class PomodoroCubit extends Cubit<PomodoroConfigState> {
  ...
  // Check and request permissions for exact alarms
  Future<bool> _checkAndRequestAlarmPermission() async {
    ...
    // For iOS or other platforms, return true without requesting anything
    return true;
  }

  // Show in-app notification when we can't schedule exact alarms
  void _showFallbackNotification(BuildContext context) {
    // Use a local timer instead of scheduled notification
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
            'Exact alarm permission not granted. Timer will use in-app notifications only.'),
  ...
  Future<void> schedulePomodoroNotification(int seconds,
    ...
    // Only check permissions on Android platform
    bool hasPermission = true;
    if (Platform.isAndroid) {
      hasPermission = await _checkAndRequestAlarmPermission();

      if (!hasPermission) {
        // If context is provided, show a fallback notification
        if (context != null) {
    ...
    try {
      ...
      // If context is provided, show fallback notification
      if (context != null) {
        _showFallbackNotification(context);
      }
    }
